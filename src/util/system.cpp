...
const char * const BITCOINVG_CONF_FILENAME = "bitcoinvg.conf";
const char * const BITCOINVG_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bitcoinvg/bitcoinvg/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinvg-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BITCOINVG_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "bitcoinvg";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitcoinVG
    // macOS: ~/Library/Application Support/BitcoinVG
    // Unix-like: ~/.bitcoinvg
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinVG";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinVG";
#else
    // Unix-like
    return pathRet / ".bitcoinvg";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINVG_CONF_FILENAME);
