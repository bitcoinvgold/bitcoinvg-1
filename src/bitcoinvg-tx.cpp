...
static void SetupBitcoinVGTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinvg-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitcoinvg-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinvg transaction\n" +
            "or:     bitcoinvg-tx [options] -create [commands]   Create hex-encoded bitcoinvg transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinvg transaction
