...
The BitcoinVG Core daemon can be configured to act as a trusted "border
router", implementing the bitcoinvg wire protocol and relay, making
...
The ZeroMQ feature in BitcoinVG Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitcoinvgd:
...
For instance:
    ...
    $ bitcoinvgd -zmqpubhashtx=tcp://127.0.0.1:232949 \
               -zmqpubhashtx=tcp://192.168.1.2:232949 \
               -zmqpubrawtx=ipc:///tmp/bitcoinvgd.tx.raw \
...
These options can also be provided in bitcoinvg.conf.
...
From the perspective of bitcoinvgd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitcoinvgd directly. Furthermore, no information is
...
using. BitcoinVGd appends an up-counting sequence number to each
