...
###   bitcoinvgcore.org and bitcoinvg.org and compares them.
...
WORKINGDIR="/tmp/bitcoinvg_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinvgcore.org"
HOST2="https://bitcoinvg.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinvg-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinvg-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "bitcoinvg.org failed to provide signature file, but bitcoinvgcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinvg.org and bitcoinvgcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinVG Core binary release signing key installed?"
   fi
