...
(define* (make-bitcoinvg-cross-toolchain target
                                  #:key
...
desirable for building BitcoinVG Core release binaries."
  ...
  (let ((target (getenv "HOST")))
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "riscv64-linux-")
           (list (make-bitcoinvg-cross-toolchain "riscv64-linux-gnu"
                                               #:base-gcc-for-libc gcc-7)))
          ((string-contains target "-linux-")
           (list (make-bitcoinvg-cross-toolchain target)))
          (else '())))))
